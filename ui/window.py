...
class JarvisApp:
    ...
    def _run_response(self, text: str):
        try:
            ...
            full = stream_response(text, memory, streamer.add)
            streamer.flush()
            self._last_response = full
            self._push_js("window.jarvisEndJarvisMessage()")

            # ── Execute SKILL: directives ──────────────────────────────
            skill_results = run_skills(full, confirm_fn=self.confirm_delete)
            good_outputs  = []
            ...
            # ── Si los skills devolvieron datos → segunda llamada al AI
            if good_outputs:
                data_str = "\n\n".join(good_outputs)
                self._push_js("window.jarvisBeginJarvisMessage()")

                follow = answer_with_data(text, data_str, memory, streamer.add)
                streamer.flush()
    ...
    def confirm_delete(self, path: str) -> bool:
        ...
        self._push_js(f"window.jarvisShowConfirm({json.dumps(path)})")
        ev.wait(timeout=60)
        return result[0]
